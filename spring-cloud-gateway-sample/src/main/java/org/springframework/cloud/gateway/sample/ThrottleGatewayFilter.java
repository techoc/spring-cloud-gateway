 ...
 * 限流网关过滤器示例实现。
 * <p>
 * 本过滤器基于 Token Bucket（令牌桶）算法实现请求限流功能。 令牌桶算法是一种常用的流量控制算法，它允许一定程度的突发流量， 同时保持长期的平均速率限制。
 * <p>
 * 工作原理：
 * <ol>
...
public class ThrottleGatewayFilter implements GatewayFilter {
	...
	/**
	 * 令牌桶的容量，表示桶中最多可以存储的令牌数量。 容量越大，允许的突发流量越高。
	 */
	int capacity;
	 ...
	 * 执行限流过滤逻辑。
	 * <p>
	 * 每次请求时创建一个新的令牌桶（实际生产环境应该按 key 缓存令牌桶）， 尝试从桶中消费一个令牌。如果消费成功，请求继续传递到过滤器链；
	 * 如果消费失败（桶中无令牌），返回 429 状态码并结束请求。
	 * <p>
	 * 注意：当前实现每次请求都创建新的令牌桶，这只是示例代码。 生产环境应该根据客户端标识（如 IP、用户 ID 等）缓存和复用令牌桶。
	 * @param exchange 当前的服务器 Web 交换对象，包含请求和响应信息
	 * @param chain 网关过滤器链，用于将请求传递给下一个过滤器
	 * @return {@link Mono}{@code <Void>} 表示异步处理完成的信号
	 */
	@Override
