 ...
 * </p>
 * <p>
 * 使用命令行快速测试（需要忽略自签名证书）： <pre>curl -i --insecure https://localhost:8443/hello</pre>
 * </p>
 *
 * @author Spencer Gibb
 */
...
@LoadBalancerClients({@LoadBalancerClient(name = "myservice", configuration = Http2Application.MyServiceConf.class), // 注册
		// myservice
		// 负载均衡客户端
		@LoadBalancerClient(name = "nossl", configuration = Http2Application.NosslServiceConf.class) // 注册
		// nossl
		// 负载均衡客户端
})
public class Http2Application {
	 ...
	 * <p>
	 * 将 "myservice" 映射到本地随机分配的 HTTPS 端口（{@code local.server.port}）， 即指向网关自身，实现环回测试。服务实例启用
	 * TLS（{@code secure=true}）。
	 * </p>
	 */
	 ...
	 * <p>
	 * 从系统属性 {@code nossl.port} 读取端口，默认值为 8080， 用于测试网关将 HTTPS 请求代理到非 TLS 后端的场景（TLS 终止）。
	 * </p>
	 */
