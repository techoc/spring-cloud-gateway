 ...
 * <p>
 * 该类作为一个轻量级的 HTTP（非 HTTPS）后端服务，配合 {@link NosslTests} 使用， 用于验证 Spring Cloud Gateway
 * 能够接受客户端的 HTTPS 请求（TLS 终止）， 然后将其以普通 HTTP 协议转发给不支持 TLS 的后端服务。
 * </p>
 * <p>
 * 该类在测试中以独立的 Spring Boot 应用启动，监听通过系统属性 {@code nossl.port} 指定的端口。
 * </p>
 */
...
public class NosslConfiguration {
	 ...
	 * <p>
	 * 用于验证网关将 HTTPS 请求正确转发到该非 TLS 后端后， 响应能够成功返回到客户端。
	 * </p>
